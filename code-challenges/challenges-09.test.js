...
const checkValues = (obj, value) => {
  ...
  for(let property in obj){
    return (obj[property] === value)? result = true: result = false;
  }
  return result;
};
...
const getHouses = (arr) => {
  ...
  arr.map( element => {
    Object.keys(element).forEach(item => {
      return (item === 'house')? houses.push(element[item]):null;
    });
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let result;
  arr.map(element => {
    let checkCharacter = Object.values(element).includes(character);
    let checkChildren = element.children;
    (checkCharacter && (checkChildren))? result = true: null;
  });
  return result;
};
