...
const addValues = (arr) => {
  return arr.reduce( ( acc, currentVal )=> {return acc + currentVal;}, 0 );
};
...
const addPurchases = (arr) => {
  return arr.reduce( (acc, currentVal) => {
    return acc + currentVal.purchasePrice;
  },0 );
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce( (acc,currentVal,idx) => {
    let totalIdx = 1;
    return totalIdx += idx;
  });
};
...
const returnNames = (arr) => {
  let localArray = [];
  arr.reduce( (acc, val) => {
    return localArray.push((val.name));
  }, []);
  return localArray;
};
...
const reversedString = (str) => {
  return str.split('').reduce( (priorEl,newEl) => {
    return newEl + priorEl;
  },'');
};
