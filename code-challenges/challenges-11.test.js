...
function transformToLis(obj){
  ...
  for(let property in obj){
    arrList.push(`<li>${property}: ${obj[property]}</li>`);
  }
  return arrList;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arrays => arrays.filter(element => {
    console.log(element);
    return (element % 5 === 0 && (element === typeof number));
  }).map(num => {
    console.log(num);
  }));
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach( object => {
    ...
    if(object['gender'] === 'male'|| object['gender'] ==='female'){
      chars.push(`${object['name']}`);
    }
  });
  let string = chars.join(' and ');
  return string;
};
